package zelix.cc.client.inGameGui.altLogin.source;
...
import zelix.cc.client.inGameGui.altLogin.source.impl.*;
...
import zelix.cc.client.utils.font.FontRenderers;
...
public class GuiAccountManager extends GuiScreen {
    ...
    public void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 2:
                ...
                Zelix.getInstance().getAltManager().getAccounts().remove(selectedAlt);
                status = "\247aRemoved.";
            ...
            case 4:
                ...
                break;
            case 5:
                Account randomAlt = Zelix.getInstance().getAltManager().getAccounts().get(new java.util.Random().nextInt(Zelix.getInstance().getAltManager().getAccounts().size()));
            ...
            case 6:
                ...
                break;
            case 7:
                Account lastAlt = Zelix.getInstance().getAltManager().getLastAlt();
                if (lastAlt == null) {
                ...
                break;
            case 8:
                Zelix.getInstance().getAltManager().getAccounts().clear();
                Zelix.getInstance().getAltManager().getAltSaving().loadFile();
                status = "\247bReloaded!";
                break;
            case 9:
                ...
                break;
            case 10:
                if (!Zelix.getInstance().getAltManager().getAccounts().isEmpty()) {
                    Zelix.getInstance().getAltManager().getAccounts().clear();
                    Zelix.getInstance().getAltManager().getAltSaving().saveFile();
                }
                break;
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        Minecraft.getMinecraft().fontRendererObj.drawString(mc.getSession().getUsername(), 10, 10, 0xDDDDDD);
        FontRenderers.drawCenteredString("Account Manager - " + Zelix.getInstance().getAltManager().getAccounts().size() + " alts", width / 2, 10, -1);
        FontRenderers.drawCenteredString(loginThread == null ? status : loginThread.getStatus(), width / 2, 20, -1);
        ...
        for (Account alt : Zelix.getInstance().getAltManager().getAccounts()) {
            if (isAltInArea(y)) {
                String name;
                ...
                FontRenderers.drawCenteredString(name, width / 2, y - offset, -1);
                FontRenderers.drawCenteredString(pass, width / 2, y - offset + 10, 5592405);
                y += 26;
            }
        }
