...
public class GuiAddAccount extends GuiScreen {
    ...
    private class AddAltThread extends Thread {
		...
		public void run() {
            if (password.equals("") && combined.getText().isEmpty()) {
                Zelix.getInstance().getAltManager().getAccounts().add(new Account(username, ""));
	...
	public void drawScreen(int i, int j, float f) {
        ...
        FontRenderers.drawCenteredString( "Alt Login", width / 2, 20, -1);
        FontRenderers.drawCenteredString(status, width / 2, 29, -1);
        ...
        FontRenderers.drawCenteredString( EnumChatFormatting.RED + "OR", width / 2, var3 + 26, -1);
