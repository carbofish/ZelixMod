...
extends GuiScreen {
        ...
        if (clickedButton.enabled) {
            if (clickedButton.id == 0) {
                ...
                try {
                    ...
                    connected = true;
                    ProxyConfig.connect(parts[0],Integer.parseInt(parts[1]));
                }
                catch (Exception e) {
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        this.mc.fontRendererObj.drawString("Proxies Reloaded", this.width / 2, 20, 16777215);
        this.mc.fontRendererObj.drawString("(SOCKS5 Proxies only)", this.width / 2, 30, 16777215);
        this.mc.fontRendererObj.drawStringWithShadow("IP:Port", this.width / 2 - 100, 47.0f, 10526880);
        this.mc.fontRendererObj.drawString(this.error, this.width / 2, 95, 16711680);
        String currentProxy = "";
        if (connected) {
